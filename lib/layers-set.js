...
const setLayers = async (region) => {
  const functionName = readlineSync.question('> Lambda function name: ', inputOpt)
  const layers = readlineSync.question('> Layers to include [layers1, layers2, ...]: ', inputOpt)
  const separator = layers.includes(',') ? ',' : ' '
  const layersArray = layers.split(separator).map((item) => item.trim())
  const strArns = await getLayers(region, layersArray)
  if (!readlineSync.keyInYN(`Update function ${functionName} using selected layers?`)) { return false }
  try {
